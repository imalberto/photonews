...
function rehydrateClientApp(config) {
    ...
    this.yui.use('store', 'import', 'handlebars-helpers', function (Y) {
        var store = new Y.Store(DATA);
            routes = Y.config.global.ROUTES,
            activeRoute = routes[Y.config.global.activeRoute];
            controllerModuleName = 'controllers/' + (activeRoutes.annotations.controller || 'main');

        // importing controller script to get the app running
        Y.import(controllerModuleName, function (controller) {

            var yaf = controller['default']({ store: store });
            ...
            function context(req, res, next) {
                req.annotations = req.route.annotations;
                req.store = store;
                req.controller = yaf;
