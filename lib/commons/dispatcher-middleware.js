...
YUI.add('dispatcher-middleware', function (Y) {
    ...
    Y.PNDispatcher = function (name) {
        ...
        return function (req, res) {
            ...
            var RouteClass = Y.routeFor(name),
                ControllerClass = Y.controllerFor(name),
                ModelClass = Y.modelFor(name),
                // RouteClass = Y.Routes[classify(name + '-route')],
...
// Router is reponsible for hooking up the correct model, the
            ...
            if (ControllerClass) {
                ...
                // TODO same thing - create a new controller based on `name`
                // controller = new Y.Controllers.BaseController(config);
                controller = Y.controllerFor('base')(config);
            }
